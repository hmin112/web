...
export const EventDetail = ({ onNavigate, isAdmin, isLoggedIn, event, onDelete, user, setEvent }: any) => {
    ...
    if (isLoggedIn && user?.loginId && event?.id) {
      const fetchEventDetail = async () => {
        try {
          ...
          const response = await axios.get(`http://localhost:8080/api/events/${event.id}?loginId=${user.loginId}`);
  ...
  const handleLikeClick = async () => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:8080/api/events/${event.id}/like`, {
