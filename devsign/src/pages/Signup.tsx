...
export const Signup = ({ onNavigate }: { onNavigate: (page: string) => void }) => {
  ...
  const handleCheckId = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:8080/api/members/check/${formData.userId}`);
  ...
  const handleSendDiscordCode = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:8080/api/members/discord-send", {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:8080/api/members/verify-code", {
        discordTag: discordTag,
