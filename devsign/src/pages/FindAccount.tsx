...
export const FindAccount = ({ onNavigate }: { onNavigate: (page: string) => void }) => {
  ...
  const handleRequestDM = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:8080/api/members/find-discord-by-info", {
        name: formData.name,
      ...
      if (res.data.status === "success") {
        ...
        // 봇 서버에 인증번호 발송 요청
        const botRes = await axios.post("http://localhost:8080/api/members/discord-send", {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:8080/api/members/verify-id-pw", {
        name: formData.name,
        studentId: formData.studentId,
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const res = await axios.put(`http://localhost:8080/api/members/reset-password-final`, {
        name: formData.name,
        studentId: formData.studentId,
