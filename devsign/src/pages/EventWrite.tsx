...
export const EventWrite = ({ onNavigate, onSave, event, fetchEvents, user }: any) => {
  ...
  const handlePublish = async () => {
    ...
    try {
      // ✨ 통합 로그 기록을 위해 loginId를 포함하여 전송합니다.
      const payload = {
        ...formData,
      ...
      let response;
      if (event && event.id) {
        response = await axios.put(`http://localhost:8080/api/events/${event.id}`, payload);
      } else {
        response = await axios.post("http://localhost:8080/api/events", payload);
      }
            ...
            {["학술", "친목", "대회", "기타"].map((c) => (
              <button 
                ...
                className={`px-6 py-2.5 rounded-full text-xs font-black transition-all ${
                  formData.category === c ? "bg-indigo-600 text-white shadow-md shadow-indigo-100" : "bg-slate-50 text-slate-400 hover:bg-slate-100"
                }`}
              >
                {c}
              </button>
