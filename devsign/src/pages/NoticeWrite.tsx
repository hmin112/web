  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        content,
        tag: tag,
        category: tag, // 백엔드 NoticeController의 필드명 대응
        images,
        loginId: user?.loginId // 누가 작성/수정했는지 로그 기록용
      };

      let response;
      if (notice && notice.id) {
        // ✨ 수정 모드 (PUT)
        response = await axios.put(`http://localhost:8080/api/notices/${notice.id}`, payload);
      } else {
        // ✨ 등록 모드 (POST)
        response = await axios.post("http://localhost:8080/api/notices", payload);
      }
              ...
              <div className="flex gap-3">
                {["공지", "행사", "모집"].map(t => (
                  <button 
                    ...
                    type="button" 
                    onClick={() => setTag(t)} 
                    className={`px-8 py-4 rounded-2xl font-black text-sm border transition-all ${
                      tag === t ? "bg-slate-900 border-slate-900 text-white shadow-lg" : "bg-white border-slate-100 text-slate-400 hover:border-slate-300"
                    }`}
                  >
                    {t}
                  </button>
