...
export const MyPageTab = ({ loginId }: { loginId: string }) => {
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      // ✅ 제출 현황 및 제목 로드
      const res = await axios.get(`http://localhost:8080/api/assembly/my-submissions`, {
        params: { loginId, year: selectedTerm.year, semester: selectedTerm.semester }
      });
      
      // ✅ 관리자가 설정한 제출 기간 로드
      const periodRes = await axios.get(`http://localhost:8080/api/admin/periods/${selectedTerm.year}`);
  ...
  const handleSaveProjectTitle = async () => {
    ...
    try {
      await axios.post(`http://localhost:8080/api/assembly/project-title`, {
        loginId, 
        year: selectedTerm.year, 
        semester: selectedTerm.semester, 
        title: projectTitle
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8080/api/assembly/submit`, formData);
