...
export const AdminPeriodTab = () => {
  ...
  const fetchPeriods = async (year: number) => {
    setIsLoading(true);
    try {
      const res = await axios.get(`http://localhost:8080/api/admin/periods/${year}`);
  ...
  const fetchSubmittedMembers = async (period: MonthPeriod) => {
    ...
    setSelectedUserIds([]);
    try {
      const res = await axios.get(`http://localhost:8080/api/admin/periods/submissions`, {
  ...
  const handleSaveAll = async () => {
    setIsLoading(true);
    try {
      await axios.post(`http://localhost:8080/api/admin/periods/save-all`, periods);
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:8080/api/admin/periods/download-zip`, {
                ...
                {isDetailLoading ? (
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {submittedMembers.map((member) => (
                      <div 
                        key={member.loginId}
                        className={`flex flex-col md:flex-row items-center justify-between p-5 rounded-3xl border transition-all ${
                          selectedUserIds.includes(member.loginId) 
