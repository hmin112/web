...
export const AdminPage = () => {
  ...
  const fetchAdminData = async () => {
    try {
      const memberRes = await axios.get("http://localhost:8080/api/admin/members");
      const logRes = await axios.get("http://localhost:8080/api/admin/logs");
  ...
  const toggleSuspension = async (id: number) => {
    try {
      const res = await axios.put(`http://localhost:8080/api/admin/members/${id}/suspend`);
  ...
  const restoreMember = async (member: Member) => {
    try {
      const res = await axios.post("http://localhost:8080/api/admin/members/restore", member);
      if (res.data.status === "success") {
  ...
  const confirmDelete = async () => {
    if (adminPassword === "1234") { 
      try {
        await axios.delete(`http://localhost:8080/api/admin/members/${memberToDelete?.id}?hard=${isHardDelete}`);
        if (isHardDelete) {
  ...
  const handleDiscordSync = async () => {
    setIsSyncing(true);
    try {
      const res = await axios.get("http://localhost:8080/api/admin/sync-discord");
      if(res.data.status === "success") {
