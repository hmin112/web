...
export const ProfilePage = ({ onNavigate, user, setUser, posts = [] }: any) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.put(`http://localhost:8080/api/members/update/${user.loginId}`, {
        dept: userInfo.major,
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:8080/api/members/change-password/${user.loginId}`, {
        currentPassword,
        newPassword
      });
